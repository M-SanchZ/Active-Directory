...
```powershell
# PowerShell script to create multiple users
$users = @(
    @{FirstName="John"; LastName="Smith"; Username="john_smith"; Password="Cyberlab123!"},
    @{FirstName="Alice"; LastName="Johnson"; Username="alice_johnson"; Password="Cyberlab123!"},
    @{FirstName="Bob"; LastName="Brown"; Username="bob_brown"; Password="Cyberlab123!"}
)

foreach ($user in $users) {
    New-ADUser -SamAccountName $user.Username -UserPrincipalName "$user.Username@mydomain.com" `
        -Name "$($user.FirstName) $($user.LastName)" -GivenName $user.FirstName `
        -Surname $user.LastName -DisplayName "$($user.FirstName) $($user.LastName)" `
        -PasswordNeverExpires $true -AccountPassword (ConvertTo-SecureString -AsPlainText $user.Password -Force) `
        -Enabled $true -PassThru | Move-ADObject -TargetPath "OU=_EMPLOYEES,DC=mydomain,DC=com"
}
